/* Software CRT for the video, based on js/tv.js: colour bars through VHS
   colour smear (in graded levels, so it can bleed in gradually), snow and
   head-switching jitter, plus full-frame analogue static. */
(function () {
  const V = window.VHS;
  const TW = 192, TH = 144;
  const LEVELS = 6;
  ...
  for (let i = 0; i < LEVELS; i++) {
    const k = i / (LEVELS - 1);
    LV.push(vhsify(raw, Math.round(V.lerp(3, 11, k)), Math.round(V.lerp(1, 5, k)), k < 0.5 ? 1 : 2));
  }
   ...
   * o.smear → 0..1 colour-under strength, o.t → seconds.
   */
  function render(o) {
    const src = LV[Math.round(V.clamp(o.smear || 0) * (LEVELS - 1))];
    const d = img.data;
    const hsRows = 6;
    for (let y = 0; y < TH; y++) {
  ...
  // full-frame analogue snow
  const SW = 320, SH = 180;
  const sc = document.createElement('canvas');
  sc.width = SW;
  sc.height = SH;
  const sx = sc.getContext('2d');
  const simg = sx.createImageData(SW, SH);
  function snowFrame() {
    const d = simg.data;
    for (let y = 0; y < SH; y++) {
      const band = Math.random() < 0.04 ? 40 : 0;
      for (let x = 0; x < SW; x++) {
        const v = Math.min(255, Math.random() * Math.random() * 330 + band);
        const i = (y * SW + x) * 4;
  ...
  V.crt = {
    ...
    draw(ctx, x, y, w, h, o) {
      const c = render(o);
      ctx.save();
      V.rr(ctx, x, y, w, h, o.radius == null ? 26 : o.radius);
      ctx.clip();
    ...
    /** Full analogue static in (x, y, w, h). */
    snow(ctx, x, y, w, h) {
      const c = snowFrame();
      ctx.save();
      ctx.imageSmoothingEnabled = true;
